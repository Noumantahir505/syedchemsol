...
export const authenticateGoogleDrive = (): Promise<boolean> => {
...
export const saveData = async (key: string, data: any): Promise<boolean> => {
  ...
  const driveSuccess = await saveToGoogleDrive(`${key}.json`, data)
  if (driveSuccess) {
    return true
  }
...
export const loadData = async (key: string): Promise<any> => {
  // Try Google Drive first
  const driveData = await loadFromGoogleDrive(`${key}.json`)
  if (driveData) {
    return driveData
  }
